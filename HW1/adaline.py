...
def check_single_sample(sample,weights,t,alpha):
    I = weights.T.dot(sample)
    y =activation_function(I)
    weights = weights + alpha * (t - I) * sample
    return weights

def check_all_samples(samples,weights,t_arr,alpha):
    for i in range(samples.shape[0]):
        weights = check_single_sample(samples[i,:],weights,t_arr[i],alpha)
    return weights

def adaline(samples,t_arr,max_iter,alpha):
    iterate = 0
    ...
    for i in range(max_iter):
        ...
        weights = check_all_samples(samples,weights,t_arr,alpha)
        ...
        if (np.sum(abs(weights - old_weights))) < 0.01:
            print(f"stopped after {i} iteration")
            break
    iterate = i
    plt.plot(range(iterate+1),error_train)
    plt.title("Train set error - adaline")
    plt.xlabel("Epoch")
    plt.ylabel("error")
    plt.show()
    plt.plot(range(iterate + 1), error_valid)
    plt.title("Validation set error - adaline")
    plt.xlabel("Epoch")
    plt.ylabel("error")
    plt.show()
    plt.plot(range(iterate + 1), mse_train)
    plt.title("Train set MSE - adaline")
    plt.xlabel("Epoch")
    plt.ylabel("mse")
    plt.show()
    plt.plot(range(iterate + 1), mse_valid)
    plt.title("Validation set MSE - adaline")
    plt.xlabel("Epoch")
    plt.ylabel("mse")
    plt.show()
    return weights,iterate

weights,iterate = adaline(X_train,y_train,10000,0.01)
